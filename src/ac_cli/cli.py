...
"""Acceptance Criteria summary updater and fast interactive status editor.

This tool keeps per-file acceptance status summaries synchronized and provides
an optimized, keyboard-driven workflow for updating criterion statuses across
the docs/acceptance-criteria catalog.

Core behavior:
- Scans all markdown files under docs/acceptance-criteria.
...
- Select file, then criterion, then status.
- Fast single-key input via click.getchar() for menu navigation.
- Paging keys:
    n = next page, p = previous page, r = back, q = quit.
- Criterion-level next/prev shortcuts at status menu:
    n = next criterion, p = previous criterion (history-aware).
- Optional sort toggles (s) at file and criterion selection levels.
...
- Update a single criterion by id/status (optionally scoped by file).
- Update multiple criteria in one command via repeated --set AC-ID=STATUS.
...
- Non-interactive single update:
    ac-cli \
            --set-criterion-id AC-TELEMETRY-LOG-001 \
            --set-status implemented
- Non-interactive bulk update:
    ac-cli \
            --set AC-STEELTARGET-AUDIO-004=implemented \
            --set AC-STEELTARGET-AUDIO-005=desktop-verified
- Non-interactive batch update from file:
    ac-cli \
            --set-file tmp/ac-updates.jsonl

Notes:
- This script expects markdown AC sections to use "### AC-..." headers and
    "- **Status:** ..." lines.
...
CRITERION_HEADER_PATTERN = re.compile(r"^###\s+(?P<id>AC-[A-Z0-9-]+):\s*(?P<title>.+?)\s*$")
...
def parse_criteria(path: Path) -> list[dict[str, object]]:
    lines = path.read_text(encoding="utf-8").splitlines()
    criteria: list[dict[str, object]] = []
    current: dict[str, object] | None = None

    for index, line in enumerate(lines):
        header_match = CRITERION_HEADER_PATTERN.match(line)
        if header_match:
            current = {
...
def find_criterion_by_id(path: Path, criterion_id: str) -> dict[str, object] | None:
    target = criterion_id.strip().upper()
    for criterion in parse_criteria(path):
        if str(criterion["id"]).upper() == target:
            return criterion
    return None


def extract_criterion_block(path: Path, criterion_id: str) -> str:
    lines = path.read_text(encoding="utf-8").splitlines()
    start_index: int | None = None
    target = criterion_id.strip().upper()

    for index, line in enumerate(lines):
        match = CRITERION_HEADER_PATTERN.match(line)
    ...
    for index in range(start_index + 1, len(lines)):
        if CRITERION_HEADER_PATTERN.match(lines[index]):
            end_index = index
            break
...
def print_criterion_panel(path: Path, criterion: dict[str, object], repo_root: Path) -> None:
    criterion_id = str(criterion["id"])
    title = str(criterion["title"])
    status = str(criterion.get("status") or "")
    criterion_text = extract_criterion_block(path, criterion_id)
    term_width = shutil.get_terminal_size(fallback=(120, 24)).columns
...
def interactive_update_loop(
    ...
    while True:
        ...
        if not existing_paths:
            click.echo("No acceptance-criteria domain files found.")
            return 1
        ...
        while True:
            raw_criteria = parse_criteria(selected_path)
            ...
            if criterion_index is None:
                ...
                criterion_choice = select_from_menu(
                    f"Select criterion in {selected_path.relative_to(repo_root).as_posix()}",
                    criterion_options,
                    zebra=True,
            ...
            print_criterion_panel(selected_path, selected_criterion, repo_root)
            ...
            status_choice = select_from_menu(
                ...
                extra_keys_help={"n": "next-ac", "p": "prev-ac"},
            ...
            if status_choice == "nav-prev":
                if history_pos > 0:
                    ...
                    click.echo("Already at first AC.")
                continue
            if status_choice == "nav-next":
                if history_pos < len(history) - 1:
                    ...
                    click.echo("Already at last AC in file.")
                continue
            ...
            # Auto-advance: move to next AC in the sorted list (wraps to criterion menu at end).
            raw_criteria_after = parse_criteria(selected_path)
            ...
            if new_idx < len(criteria_after) - 1:
                ...
                if criteria_after:
                    # Wrap to first AC instead of jumping up to the criterion menu level.
                    criterion_index = 0
                    del history[history_pos + 1:]
                    history.append(criterion_index)
                    history_pos = len(history) - 1
...
def filtered_interactive_loop(
    ...
    while True:
        ...
        refreshed = find_criterion_by_id(path, str(criterion["id"]))
        if refreshed:
            criterion = refreshed

        click.echo(click.style(f"\n[{index + 1}/{len(flat_list)}]", dim=True))
        print_criterion_panel(path, criterion, repo_root)
        ...
        # If the status changed away from target_status this AC drops out of the list;
...
def lookup_criterion_interactive(
    ...
    """Find a single AC by id, show its panel, present the status menu once, and exit."""
    criterion_id = criterion_id.strip().upper()
    matches: list[tuple[Path, dict[str, object]]] = []
    for path in domain_files:
        c = find_criterion_by_id(path, criterion_id)
        if c:
            matches.append((path, c))

    if not matches:
        raise click.ClickException(f"Criterion '{criterion_id}' not found in acceptance-criteria docs.")
    ...
    while True:
        ...
        refreshed = find_criterion_by_id(path, criterion_id)
        if refreshed:
            criterion = refreshed
...
def apply_status_change_by_id(
    ...
    for path in target_paths:
        criterion = find_criterion_by_id(path, criterion_id)
        if criterion:
    ...
    if not matches:
        if file_filter:
            raise click.ClickException(
                f"Criterion '{criterion_id}' not found in {file_filter}."
            )
        raise click.ClickException(f"Criterion '{criterion_id}' not found in acceptance-criteria docs.")
...
def parse_set_entry(entry: str) -> tuple[str, str]:
    ...
    if "=" not in raw:
        raise click.ClickException(
            f"Invalid --set value '{entry}'. Expected format AC-ID=STATUS."
        )
...
def parse_batch_update_jsonl(path: Path) -> list[dict[str, str | None]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, raw in enumerate(handle, start=1):
            ...
            if not criterion_id or not status:
                raise click.ClickException(
                    f"Invalid JSONL row at {path}:{line_number}: requires criterion_id/id/ac_id and status"
                )
...
def parse_batch_update_csv(path: Path, delimiter: str = ",") -> list[dict[str, str | None]]:
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        ...
        for line_number, row in enumerate(reader, start=2):
            ...
            if not criterion_id or not status:
                raise click.ClickException(
                    f"Invalid CSV/TSV row at {path}:{line_number}: requires criterion_id/id/ac_id and status columns"
                )
...
@click.option(
    "--set-criterion-id",
    type=str,
    help="Non-interactive mode: AC criterion id to update, e.g. AC-TELEMETRY-LOG-001.",
)
@click.option(
    "--set-status",
    "set_status",
...
@click.option(
    ...
    multiple=True,
    type=str,
    help="Non-interactive bulk mode: repeat AC-ID=STATUS (for example --set AC-FOO-001=implemented).",
)
@click.option(
    "--set-file",
    "set_file_input",
    type=str,
    help="Non-interactive batch mode: path to .jsonl/.csv/.tsv with rows containing criterion_id (or id/ac_id) and status.",
)
@click.option(
    "--file",
    "set_file",
    type=str,
    help="Optional file scope for non-interactive updates, repo-relative path such as docs/acceptance-criteria/telemetry.md.",
)
@click.option(
    "--set-blocked-reason",
    type=str,
...
@click.option(
    "--filter-status",
    type=str,
    help="Filter by status: walks matching ACs interactively (default) or shows tree with --tree.",
)
@click.option(
    "--tree",
    is_flag=True,
...
@click.option(
    ...
    default=Path("."),
    show_default=True,
    help="Project root containing acceptance criteria documentation.",
)
@click.option(
    "--criteria-dir",
    type=str,
    ...
    show_default=True,
    help="Directory (absolute or relative to --repo-root) containing AC markdown files.",
)
def main(
    check: bool,
    verbose: bool,
    ...
    if not domain_files:
        print(f"No acceptance-criteria domain files found under: {criteria_dir}", file=sys.stderr)
        raise SystemExit(1)

    # Positional AC-ID lookup: find the criterion, show panel + status menu, done.
    if criterion_id:
        if check or filter_status or set_criterion_id or set_status or set_updates or set_file_input or set_file or tree:
            raise click.ClickException(
                "Positional AC-ID cannot be combined with --check, --filter-status, --tree, or --set-* options."
            )
        raise SystemExit(
    ...
    if check and changed_paths:
        if verbose:
            print(
                f"{len(changed_paths)} acceptance-criteria file(s) need summary updates.",
                file=sys.stderr,
            )
        else:
    ...
    if filter_status:
        ...
        try:
            ...
            raise
        criteria_by_file = collect_criteria_by_status(repo_root, domain_files, normalized_status)
        if tree or not interactive:
            ...
            raise SystemExit(0)
        # Interactive: walk through matching ACs one by one.
        raise SystemExit(
    ...
    if non_interactive_requested:
        ...
        if mode_count > 1:
            raise click.ClickException(
                "Use exactly one non-interactive update mode: --set-file, --set AC-ID=STATUS (repeatable), or --set-criterion-id with --set-status."
            )
